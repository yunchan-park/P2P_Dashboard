...
def train_and_evaluate(df, version_name):
    ...
    # --- 지역구별 평가 결과 엑셀 저장 ---
    try:
        gu_results_df.to_excel('../../../../인천_구별_평가_결과.xlsx', index=False)
        print("\n[INFO] '인천_구별_평가_결과.xlsx' 파일이 성공적으로 저장되었습니다.")
    except Exception as e:
        print(f"\n[ERROR] 엑셀 파일 저장 중 오류가 발생했습니다: {e}")

    return best_model_info, te, ohe, X_train_final.columns

# --- 1. 데이터 로드 및 전처리 ---
try:
    # CSV 파일 사용으로 변경
    data_path = '../../data/인천_매매_합본.csv'
    df_original = pd.read_csv(data_path, encoding='utf-8')
except FileNotFoundError:
    print(f"[ERROR] Data file not found at: {data_path}. Please ensure you have saved the excel as '인천_매매_데이터.csv' with UTF-8 encoding.")
    exit()
...
# --- 2. 모델링 실행 (사용자 의견에 따라 이상치 제거하지 않은 전체 데이터 사용) ---
best_model_info, te, ohe, columns = train_and_evaluate(df_with_features, "Final Model")
...
joblib.dump(best_model, '../../predict_model/test_ver/rf_model_incheon_prac.joblib')
joblib.dump(te, '../../predict_model/test_ver/target_encoder_incheon_prac.joblib')
joblib.dump(ohe, '../../predict_model/test_ver/onehot_encoder_incheon_prac.joblib')
joblib.dump(columns, '../../predict_model/test_ver/model_columns_incheon_prac.joblib')
