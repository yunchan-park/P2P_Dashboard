...
def train_and_evaluate(df, version_name):
    ...
    # --- 지역구별 평가 결과 엑셀 저장 ---
    try:
        gu_results_df.to_excel('../../../../울산_구별_평가_결과.xlsx', index=False)
        print("\n[INFO] '울산_구별_평가_결과.xlsx' 파일이 성공적으로 저장되었습니다.")
...
try:
    # CSV 파일 사용으로 변경
    data_path = '../../data/울산_매매_합본.csv'
    df_original = pd.read_csv(data_path, encoding='utf-8')
except FileNotFoundError:
    print(f"[ERROR] Data file not found at: {data_path}. Please ensure you have saved the excel as '울산_매매_데이터.csv' with UTF-8 encoding.")
    exit()
...
joblib.dump(best_model, '../../predict_model/test_ver/rf_model_ulsan_prac.joblib')
joblib.dump(te, '../../predict_model/test_ver/target_encoder_ulsan_prac.joblib')
joblib.dump(columns, '../../predict_model/test_ver/model_columns_ulsan_prac.joblib')
