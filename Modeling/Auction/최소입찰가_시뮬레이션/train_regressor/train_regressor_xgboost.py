# train_regressor_xgboost.py
"""
낙찰된 경매 데이터를 사용하여 '낙찰가'를 예측하는 회귀 모델을 학습합니다.
"""
import os
import sys
import numpy as np
...
RANDOM_STATE = 42
# MODEL_OUT = "../trained_model/trained_regressor_model/auction_regressor_xgboost.joblib"

# --- 데이터 준비 함수 ---
def prepare_regression_data(df):
    """회귀 모델 학습을 위한 최종 데이터셋(X, y)을 준비합니다."""
    # 사용할 변수 목록 정의 (낙찰가, label 제외)
    features = [
...
def train_xgb_regressor(X_train, y_train, X_val, y_val):
    """XGBoost 회귀 모델을 학습시킵니다. (Native API 사용)"""
    params = {
...
def evaluate_regressor(model, X, y):
    """학습된 회귀 모델의 성능을 평가합니다. (Native API)"""
    dtest = xgb.DMatrix(X)
...
def main():
    ...
    # 5. 모델 학습
    print("\n[INFO] 회귀 모델 학습 시작...")
    model = train_xgb_regressor(X_train, y_train, X_test, y_test)
